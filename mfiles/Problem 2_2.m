...
[anwers_mah,anwers_euc,anwers_bayes] = classifier(num,[1/3,1/3,1/3],X1,mu1,mu2,mu3,sigma1,sigma2,sigma3);
...
function [anwers_mah,anwers_euc,anwers_bayes] = classifier(num,P,X1,mu1,mu2,mu3,sigma1,sigma2,sigma3)
    ...
    anwers_mah = [];
    for i=1:num
        g1_bayes = ((2*pi).^(-2/2))*(det(sigma1).^(1/2))*( exp((-1/2)*(X1(i,:)-mu1)*inv(sigma1)*(X1(i,:)-mu1)') );  %(1/4)*(X1(i,:)-mu1)*(X1(i,:)-mu1).';
        g2_bayes = ((2*pi).^(-2/2))*(det(sigma2).^(1/2))*( exp((-1/2)*(X1(i,:)-mu2)*inv(sigma2)*(X1(i,:)-mu2)') );
        g3_bayes = ((2*pi).^(-2/2))*(det(sigma3).^(1/2))*( exp((-1/2)*(X1(i,:)-mu3)*inv(sigma3)*(X1(i,:)-mu3)') );
